...
--
-- Migration is idempotent: if duplicates exist we skip adding the unique index
...
BEGIN
    ...
    IF duplicate_count > 0 THEN
        RAISE NOTICE
            'questions_unique_lower_text_per_bank: skipped because % duplicate group(s) exist in public.questions. Resolve duplicates before re-running this migration.',
            duplicate_count;
        RETURN;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        CREATE UNIQUE INDEX uq_questions_bank_lower_text
            ON public.questions (
                question_bank_id,
                LOWER(question_text)
            );
...
COMMENT ON INDEX public.uq_questions_bank_lower_text
