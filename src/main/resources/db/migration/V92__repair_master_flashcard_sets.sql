...
-- Legacy master versions created by V39 may have NULL created_by, so fall back to
-- the course owner/assigned SME before using one active staff account as repair actor.
...
repair_source AS (
    SELECT
        ...
        version.course_id,
        COALESCE(version.created_by, course.creator_id, course.assigned_sme_id, fallback_actor.id) AS created_by,
        lesson.title
