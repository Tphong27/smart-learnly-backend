...
public class SePayWebhookSignatureVerifier {
    ...
    @Autowired
    // Khởi tạo verifier production với đồng hồ UTC của hệ thống.
    public SePayWebhookSignatureVerifier(SystemSettingsService systemSettingsService) {
        this(systemSettingsService, Clock.systemUTC());
    }

    // Cho phép test cố định thời gian để kiểm tra cửa sổ chống replay 5 phút.
    SePayWebhookSignatureVerifier(SystemSettingsService systemSettingsService, Clock clock) {
        this.systemSettingsService = systemSettingsService;
        this.clock = clock;
    }

    // Xác thực timestamp và HMAC-SHA256 trên đúng raw body trước khi tin cậy payload.
    ...
    // Đọc timestamp Unix từ header và trả lỗi xác thực nếu thiếu hoặc sai định dạng.
    ...
    // Chặn webhook quá cũ hoặc ở tương lai quá giới hạn để giảm nguy cơ replay.
    ...
    // Kiểm tra tiền tố sha256 và giải mã đúng 32 byte chữ ký hexadecimal.
    ...
    // Tính HMAC trên chuỗi timestamp.rawBody theo contract webhook SePay.
