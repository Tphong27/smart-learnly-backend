...
public class GoogleIdTokenService {
    ...
    // Khởi tạo bộ xác minh Google token dùng cấu hình OAuth quản trị hiện tại.
    public GoogleIdTokenService(SystemSettingsService settingsService) {
        this.settingsService = settingsService;
    }

    // Xác minh chữ ký, issuer, hạn dùng, audience và email của Google ID token.
    public GoogleIdentity verify(String idToken) {
        String clientId = settingsService.resolveGoogleSettings().clientId();
        if (clientId == null || clientId.isBlank()) {
            throw new BusinessException(ErrorCode.EXTERNAL_SERVICE_UNAVAILABLE, "Google login is not configured");
        }
    ...
    // Chuẩn hóa lỗi token Google về cùng business error, không lộ chi tiết xác minh.
    private BusinessException invalidToken() {
        return new BusinessException(ErrorCode.INVALID_CREDENTIALS, "Google ID token is invalid");
    }

    // Tạo decoder dùng JWK chính thức và chỉ chấp nhận hai issuer hợp lệ của Google.
    private JwtDecoder googleDecoder() {
        NimbusJwtDecoder jwtDecoder = NimbusJwtDecoder.withJwkSetUri(
                "https://www.googleapis.com/oauth2/v3/certs").build();
