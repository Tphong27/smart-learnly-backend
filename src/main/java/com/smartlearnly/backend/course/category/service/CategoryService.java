package com.smartlearnly.backend.course.category.service;
...
public class CategoryService {
    ...
    // Tải category quản trị theo keyword, trạng thái và category cha.
    ...
    // Tải category công khai đang hoạt động theo keyword và category cha.
    @Transactional(readOnly = true)
    ...
    // Trả chi tiết category hoặc báo không tồn tại.
    @Transactional(readOnly = true)
    public CategoryResponse get(UUID categoryId) {
        return toResponse(findCategory(categoryId));
    }

    // Tạo category, chuẩn hóa slug/hierarchy và ghi audit.
    @Transactional
    ...
    // Cập nhật đúng các trường category được cung cấp và ghi audit.
    @Transactional
    ...
    // Xóa category khi chưa có category con hoặc khóa học tham chiếu.
    @Transactional
    ...
    // Kiểm tra category cha hợp lệ và giới hạn cây ở hai cấp.
    ...
    // Sinh slug lúc tạo và ngăn trùng toàn hệ thống.
    ...
    // Chuẩn hóa slug lúc cập nhật và bỏ qua chính category hiện tại khi kiểm tra trùng.
    ...
    // Chuyển tên tiếng Việt thành slug URL hợp lệ, có giới hạn độ dài.
    ...
    // Tìm category theo ID hoặc trả business error thống nhất.
    private Category findCategory(UUID categoryId) {
        return categoryRepository.findById(categoryId)
                .orElseThrow(() -> new BusinessException(ErrorCode.CATEGORY_NOT_FOUND));
    }

    // Chuyển entity category thành DTO API không chứa object cha lồng nhau.
    ...
    // Ghi audit thao tác category bằng email người dùng hiện tại.
    ...
    // Chuẩn hóa chuỗi bắt buộc và trả lỗi nghiệp vụ khi rỗng.
