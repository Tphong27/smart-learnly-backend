...
public record CreateCourseRequest(
        ...
        @NotBlank(message = "Course title is required") @Size(max = 255, message = "Course title must not exceed 255 characters") String title,
        @Size(max = 280, message = "Course slug must not exceed 280 characters") @Pattern(regexp = "^[a-z0-9]+(?:-[a-z0-9]+)*$", message = "Course slug can contain lowercase letters, numbers, and hyphens only") String slug,
        String shortDescription,
        String description,
        String outcomes,
        String requirements,
        @Size(max = 50, message = "Language must not exceed 50 characters") String language,
        @Size(max = 30, message = "Level must not exceed 30 characters") String level,
        ...
        @NotNull(message = "Assigned SME is required") UUID assignedSmeId) {
}
