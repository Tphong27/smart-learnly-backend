...
public class PreviewLessonService {
    ...
    // Lấy tất cả bài học được đánh dấu cho xem thử trong chương trình công khai.
    ...
    // Tìm một bài học mẫu theo mã hiện tại hoặc mã nguồn để hỗ trợ dữ liệu đã sao chép phiên bản.
    @Transactional(readOnly = true)
    ...
    // Chặn xem thử nếu khóa học chưa được xuất bản hoặc không tồn tại.
    ...
    // Chuyển bài học chương trình thành dữ liệu công khai an toàn cho màn hình xem thử.
    ...
    // Sắp xếp các chương theo thứ tự hiển thị ổn định của chương trình.
    ...
    // Chỉ giữ bài học đã xuất bản và được cho phép xem thử, rồi sắp xếp ổn định.
    ...
    // Chuẩn hóa thứ tự trống thành 0 để phản hồi luôn có giá trị số.
