...
/**
 * Covers OpenSpec task 8.2: lesson progress is keyed by
 * (classId + lessonIdentityId), so completing a lesson in one class
 * never marks the same lesson complete in a different class.
 */
...
class TraineeProgressServiceTest {
        ...
        void completingLessonInOneClassDoesNotMarkItCompleteInAnotherClass() {
                ...
                // Same lesson identity is shared by both classes because
                // the curriculum was inherited from the master curriculum.
                ...
                when(classOfferingRepository.findByIdAndDeletedAtIsNull(classA))
                                .thenReturn(Optional.of(offeringA));

                CurriculumVersion versionA = version(courseId);

                when(curriculumResolutionService.resolveTraineeLearning(
                                courseId,
                                classA,
                                studentId)).thenReturn(new CurriculumResolution(
                                                versionA,
                                                null,
                                                classA,
                                                false,
                ...
                CurriculumLesson lessonA = lesson(
                                lessonRowIdA,
                                lessonIdentityId);
                ...
                LessonProgressResponse response = traineeProgressService.updateLessonProgress(
                                lessonIdentityId,
                                courseId,
                                classA,
                                true);
                ...
                assertThat(saved.getLessonId())
                                .isEqualTo(lessonRowIdA);

                assertThat(saved.getCourseId())
                                .isEqualTo(courseId);

                assertThat(saved.getClassId())
                                .isEqualTo(classA);

                assertThat(saved.getLessonIdentityId())
                                .isEqualTo(lessonIdentityId);
